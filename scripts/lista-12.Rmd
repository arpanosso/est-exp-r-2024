...
Para o exemplo, ler o arquivo **subdividia.rds**. Não se esqueça de deixar os tratamentos como fator.

```{r}
data_set <- read_rds("../data/subdividida.rds")
glimpse(data_set)
```
...
data_set |> 
  mutate(
    trat = interaction(adubo,suplemento)
  ) |> 
  group_by(trat) |> 
  mutate(
    media = mean(prod, na.rm = TRUE),
...
adubo<- data_set |>  pull(adubo) |> as_factor()
suplemento <- data_set |>  pull(suplemento) |> as_factor()
trat <- interaction(adubo,suplemento)
...
```{r}
diagnostico |> 
  arrange(rs) |> slice(1) |> pull(rs) -> rs_outlier
...
data_set |> 
  ...
  ggplot(aes(x=as.numeric(as_factor(adubo)), y = media, 
             color=as_factor(suplemento))) +
  geom_point() +
  geom_line()
```
...
data_set |> 
  ...
  ggplot(aes(x=as.numeric(as_factor(suplemento)), y = media, 
             color=as_factor(adubo))) +
  geom_point() +
  geom_line()
```

```{r}
x<- c(20, 25, 34, 20, 25, 22, 21, 26)
mean(x)
var(x)
sd(x)

t.test(x,mu=20,alternative = "g",conf.level = 0.95)
```
