...
```{r}
# -------------- Homogeneidade das Variâncias (Transformações - Box-Cox) ---------------
# A transformação Box-Cox é uma das possíveis formas de contornar o problema de dados 
# que não obedecem os pressupostos da análise de variância, como normalidade dos dados.
require(MASS)
...
# Utilizando as funções do pacote Básico e agricolae
mod<-aov(y~trat)
anova(mod)

#Calculo do CV, 100*sqrt(QMr)/mediaGeral
GLres<-df.residual(mod) 	# graus de liberdade dos resíduos
SQres<-deviance(mod)	# soma de quadrados dos resíduos
QMres<-SQres/GLres	# quadrado médio dos resíduos
cv<-100*sqrt(QMres)/mean(y)
cv

# o teste F foi significativo ao nível de 5% de probabilidade, rejeitamos H0 e concluímos que
# as raçoes possuem efeitos diferentes quanto ao ganho de peso de animais, com um grau de confiança
# maior que 95%

# ---------------------- Comparações múltiplas ------------------------
# Quando não se tem qualquer informação a priori sobre os tratamentos, 
# sugere-se "comparações múltiplas". Os testes mais comuns para Comparações Múltiplas são: 
# Tukey; Duncan e Student Newman Keuls (SNK).
...
Tukey
comp<-HSD.test(mod,"trat",group=TRUE,alpha=0.05)$group
bar.group(comp,xlab="Rações", ylab="Ganho de Peso (kg)",ylim=c(0,50),
		main="Teste de Tukey");box()
...
# Teste de Duncan
#require(agricolae)
Duncan<-duncan.test(mod,"trat",alpha=0.05)
Duncan
comp<-duncan.test(mod,"trat",alpha=0.05)$group
bar.group(comp,xlab="Rações", ylab="Ganho de Peso (kg)",ylim=c(0,50),
		main="Teste de Duncan");box()

# Teste de Student-Newman-Keuls
#require(agricolae)
snk<-SNK.test(mod, "trat", alpha=0.05)
snk

#teste de Dunnett
#install.packages("multcomp")
require(multcomp)
Dunnett <- glht(mod, linfct = mcp(trat = "Dunnett"))
summary(Dunnett)

# Teste t-Student (LSD)
require(agricolae)
Student<-LSD.test(mod,"trat")
Student

# Teste t- Student com proteção de Bonferroni
require(ExpDes.pt)
dic(trat,y,quali=T,mcomp="lsdb")



# Teste de Scheffé
require(agricolae)
Scheffe<-scheffe.test(mod,"trat")
Scheffe
comp<-scheffe.test(mod,"trat")$groups
bar.group(comp,xlab="Rações", ylab="Ganho de Peso (kg)",ylim=c(0,50),
		main="Teste de Scheffé");box()
